#include "mGameCardsOasisPoker.h"

#include "server/mUser.h"

TOasisPokerProducer OasisPoker;

TPokerCombination* TOasisPokerRoom::fBonus;
TPokerCombination* TOasisPokerRoom::fGame;
...
TStr TOasisPokerRoom::Log(IPlayer* p)
{
	TStr s;
...
void TOasisPokerRoom::OnCommitCard( TStep* p )
{
	switch( p->fAction )
	{
		case MARKER_PUSH:
...
void TOasisPokerRoom::OnTagPlayNext(IPlayer *Player)
{
	if( Player->fTag > -1 ) {
...
void TOasisPokerRoom::OnBoxBonus(const unsigned int i, const bool)
{
...
void TOasisPokerRoom::OnTagPlay(IPlayer *Player)
{
	...
	if( Player->User()->Cash() >= Box(Player->fTag)->Bet(BOX_ANTE)*3 )
		Player->AddMsg(ssi->SSIRes("cards_button_change"));
}

// - OnGameStart

void TOasisPokerRoom::OnGameStart()
{
	CardsClear();
	CardsLoad("cards_oasispoker.txt");
	TagInit();
	bool isGame = false;
	for( unsigned int i = 0; i < 5; i++ )
		if( isBoxPlay(i) ) {
	...
	if( isGame ) {
		...
		do {
			...
			for( unsigned int i = 0; i < 5; i++ )
				if( isBoxPlay(i) ) {
					// защита от числа боксов > 2
					for( unsigned int j = 0; j < 5; j++ )
						MarkerCard(i,MARKER_CLOSE);
					TCardsCombination BoxCombination(Box(i)->Cards());
					if( BoxCombination.Combination() != CARD_NOT ) {
						unsigned int Cash =
							Box(i)->Bet(BOX_ANTE) * 2 *
							fGame->Cash(BoxCombination.Combination());
	...
	IPlayer* Player)
{
	...
	if( Action == "Change" ) {
		...
		if( Player->User()->CashBetCommit(cOasisPokerGame,Table(),CashBet) ) {
			unsigned int CardIndex = ID.ToInt();
			int i = Player->fTag;
			TCardsCombination BoxCombinationOld(Box(i)->Cards());
			int CashOld = 0;
			if( BoxCombinationOld.Combination() != CARD_NOT )
				CashOld = 
					Box(i)->Bet(BOX_ANTE) * 2 *
					fGame->Cash(BoxCombinationOld.Combination());
			do {
				Marker();
...
void TOasisPokerRoom::OnPlayerAdd(IPlayer *Player)
{
